...
struct DrawingContext {
    ...
    outline_quad_shader: glow::Program,
...
impl DrawingContext {
    fn draw_quad(&self, dims: &widgets::Rect, color: &Color) {
    ...
    fn draw_rounded_quad(&self, dims: &widgets::Rect, color: &Color, rounding: f32) {
        let window_size = self.window.drawable_size();
        unsafe {
            self.gl.use_program(Some(self.quad_shader));
            self.gl.uniform_2_f32(
    ...
    fn get_current_font(&self) -> &LoadedFont {
    ...
    fn draw_text_monospace(&self, text: &str, mut pos: IVec2, color: &Color) -> IVec2 {
        for i in text.chars() {
